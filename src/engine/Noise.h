...
public:
    ...
    static float fbm(float x, float y, int octaves = 4, float lacunarity = 2.0f, float persistence = 0.5f) {
        float value = 0.0f;
    ...
    static float ridged(float x, float y, int octaves = 4) {
        float value = 0.0f;
        float amplitude = 1.0f;
        float frequency = 1.0f;
        float weight = 1.0f;
        ...
        for (int i = 0; i < octaves; i++) {
            float signal = simplex2D(x * frequency, y * frequency);
            signal = 1.0f - std::abs(signal);
            signal *= signal * weight;
            weight = glm::clamp(signal * 2.0f, 0.0f, 1.0f);
...
private:
    ...
    static inline const int grad3[12][3] = {
