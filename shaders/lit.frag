...
void main() {
    ...
    float ambient = camera.ambientIntensity * camera.weatherAmbient;
    
    vec3 diffuse = baseColor * camera.sunColor * camera.sunIntensity * NdotL * (1.0 - shadow);
    vec3 ambientColor = baseColor * ambient;
    
    vec3 litColor = ambientColor + diffuse;
    litColor += litColor * camera.lightningFlash * 2.0;
    
    float dist = length(camera.cameraPos - fragWorldPos);
    float fogFactor = exp(-dist * camera.fogDensity);
    fogFactor = clamp(fogFactor, 0.0, 1.0);
    
    vec3 finalColor = mix(camera.fogColor, litColor, fogFactor);
